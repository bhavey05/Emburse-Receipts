...
1. First run WITHOUT `--send` to fetch the PDF and show the send plan:
   ...
   If fetching needs extra input (ClassPass Stripe URL, Verizon PDF), follow the provider flow described by the script's error message — same as `/receipts:fetch`.
